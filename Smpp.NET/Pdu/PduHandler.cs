...
namespace JulMar.Smpp.Pdu
{
    ...
    {
        private static readonly ILogger Logger = SmppLogging.CreateLogger<PduHandler>();
        ...
        public static bool Dispatch(SmppSessionState state, SmppPdu pdu)
        {
            ...
            if (typeMap_.ContainsKey(pdu.CommandId))
            {
                ...
                {
                    Logger.LogError(ex, "Invalid PDU handler cast. cmdId={CommandId}, pduType={PduType}, handlerType={HandlerType}", pdu.CommandId, pdu.GetType().FullName, handler.GetType().FullName);
                }
            }
            else
            {
                Logger.LogWarning("Received PDU with unknown command id {CommandId}, seq={Sequence}", pdu.CommandId, pdu.SequenceNumber);
            }
