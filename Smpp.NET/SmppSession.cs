...
namespace JulMar.Smpp
{
    ...
    {
        private readonly ILogger _logger = SmppLogging.CreateLogger<SmppSession>();
        ...
        protected virtual void OnReceiveData(object sender, SocketEventArgs args)
        {
            ...
            if (re.Length >= packetLength)
            {
                ...
                _logger.LogDebug("Received raw SMPP data, length = {Length}, packetLength = {PacketLength}", re.Length, packetLength);
                ...
                if (re.Length >= packetLength)
                {
                    try
                    {
                        ...
                        _logger.LogDebug("Parsed PDU: cmdId={CommandId}, seq={Sequence}, status={Status}", pdu.CommandId, pdu.SequenceNumber, pdu.Status);
                        ...
                        if (!ea.Handled)
                        {
                            ...
                            catch (InvalidSmppStateException ex)
                            {
                                _logger.LogWarning(ex, "Invalid SMPP state while processing PDU. cmdId={CommandId}, seq={Sequence}", pdu.CommandId, pdu.SequenceNumber);
                    ...
                    catch (PduException pduex)
                    {
                        ...
                        _logger.LogError(pduex, "PDU parsing/validation error. HasPdu={HasPdu}, cmdId={CommandId}, seq={Sequence}", pduex.HasPDU, pdu?.CommandId, pdu?.SequenceNumber);
                    ...
                    {
                        _logger.LogError(ex, "Unexpected error while processing incoming SMPP data. Closing socket.");
        ...
        internal bool SendPdu(SmppPdu pdu)
        {
            ...
            {
                _logger.LogWarning("Attempt to send PDU when socket is not connected. cmdId={CommandId}, seq={Sequence}", pdu.CommandId, pdu.SequenceNumber);
                return false;
            }
            ...
            {
                _logger.LogError(ex, "Error in PostProcessPdu event handlers. cmdId={CommandId}, seq={Sequence}", pdu.CommandId, pdu.SequenceNumber);
            }

            try
            {
                _logger.LogDebug("Sending PDU. cmdId={CommandId}, seq={Sequence}, status={Status}", pdu.CommandId, pdu.SequenceNumber, pdu.Status);
            ...
            {
                _logger.LogError(ex, "Socket error while sending PDU. cmdId={CommandId}, seq={Sequence}");
